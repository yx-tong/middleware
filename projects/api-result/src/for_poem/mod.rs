...
use std::borrow::Cow;
use poem_openapi::__private::poem::{IntoResponse, Response};
use poem_openapi::{ApiResponse};
use poem_openapi::registry::{MetaMediaType, MetaResponse, MetaResponses, MetaSchema, MetaSchemaRef, Registry};
use poem_openapi::types::{IsObjectType, ParseError, ParseFromJSON, ToJSON,  Type};
use serde_json::{Number, Value};
use crate::{ApiError, ApiResult};


impl<T: Type, E: ApiError + Send + Sync> ApiResponse for ApiResult<T, E> {
...
impl<T: ToJSON + Send + Type, E: ApiError + Send + Sync> IntoResponse for ApiResult<T, E> {
...
impl<T: Type, E: ApiError + Send + Sync> Type for ApiResult<T, E> {
    ...
    fn schema_ref() -> MetaSchemaRef { MetaSchemaRef::Reference(Self::name().into_owned()) }
    fn register(registry: &mut Registry) {
                ...
                properties: {
                    let mut fields = Vec::new();
                    {
                        ...
                        let patch_schema = {
                            ...
                            if let Some(field_description) = None { schema.description = Some(field_description); }
                            schema
                        };
    ...
    fn raw_element_iter<'a>(&'a self) -> Box<dyn Iterator<Item=&'a Self::RawElementValueType> + 'a> { Box::new(::std::iter::IntoIterator::into_iter(self.as_raw_value())) }
}

impl<T: Type, E: ApiError + Send + Sync> IsObjectType for ApiResult<T, E> {}

impl<T: Type + ParseFromJSON, E: ApiError + Send + Sync> ParseFromJSON for ApiResult<T, E> {
    fn parse_from_json(value: Option<Value>) -> Result<Self, ParseError<Self>> {
        match value {
            Some(Value::Object(mut map)) => {
                match ParseFromJSON::parse_from_json(map.remove("data")) {
...
impl<T: Type + ToJSON, E: ApiError + Send + Sync> ToJSON for ApiResult<T, E> {
        ...
        match self {
            ApiResult::Success(s) => {
                ...
                match ToJSON::to_json(s) {
                    Some(s) => {
                        object.insert("data".to_string(), s)
                    }
                    None => {
