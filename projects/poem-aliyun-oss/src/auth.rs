...
impl Auth for AliyunOssBucket {
    fn sign(
        ...
        let date = headers.get(DATE).and_then(|d| Some(d.to_str().unwrap_or_default())).unwrap_or_default();
        let content_type = headers.get(CONTENT_TYPE).and_then(|c| Some(c.to_str().unwrap_or_default())).unwrap_or_default();
        let content_md5 =
            headers.get("Content-MD5").and_then(|md5| Some(encode(md5.to_str().unwrap_or_default()))).unwrap_or_default();

        let mut oss_headers: Vec<(&HeaderName, &HeaderValue)> =
            headers.iter().filter(|(k, _)| k.as_str().contains("x-oss-")).collect();
        ...
        let mut hasher =
            HmacSha1::new_from_slice(key_secret.as_bytes()).expect("Hmac can take key of any size, should not happned");
...
fn get_oss_resource_str(bucket: &str, object: &str, oss_resources: &str) -> String {
    let oss_resources = if oss_resources != "" { String::from("?") + oss_resources } else { String::new() };
    if bucket == "" { format!("/{}{}", bucket, oss_resources) } else { format!("/{}/{}{}", bucket, object, oss_resources) }
}
