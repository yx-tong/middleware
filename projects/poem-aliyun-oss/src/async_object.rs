...
use super::errors::{OssError, ObjectError};
use bytes::Bytes;
use reqwest::header::HeaderMap;
use reqwest::Method;
use crate::object::ListObjects;
use crate::oss::AliyunOssBucket;
...
impl AliyunOssBucket {
    ...
    {
        let (host, headers) =
            self.build_request(Method::GET, object_name, headers, resources)?;
        println!("HOST: {}", host);
    ...
    {
        let (host, headers) =
            self.build_request(Method::PUT, object_name, headers, resources)?;
    ...
    pub async fn copy_object_from_object(&self, src: &str, dest: &str, headers: HeaderMap, resources: &HashMap<String, String>) -> Result<(), OssError> {
    ...
    pub async fn delete_object<S>(&self, object_name: &str) -> Result<(), OssError> {
        ...
        let (host, headers) =
            self.build_request(Method::DELETE, object_name, headers, &HashMap::new())?;
    ...
    pub async fn init_multi(&self, object_name: &str, headers: HeaderMap, resources: &HashMap<String, String>) -> Result<InitiateMultipartUploadResult, OssError> {
        let (host, headers) =
            self.build_request(Method::POST, object_name, headers, resources)?;
    ...
    pub async fn upload_part(&self, buf: &[u8], object_name: &str, headers: HeaderMap, resources: &HashMap<String, String>) -> Result<String, OssError> {
        let (host, headers) =
            self.build_request(Method::PUT, object_name, headers, resources)?;
    ...
    ///
    pub async fn complete_multi(&self, body: String, object_name: &str, headers: HeaderMap, resources: &HashMap<String, String>) -> Result<CompleteMultipartUploadResult, OssError> {
        let (host, headers) =
            self.build_request(Method::POST, object_name, headers, resources)?;
    ...
    pub async fn abort_multi(&self, object_name: &str, headers: HeaderMap, resources: &HashMap<String, String>) -> Result<(), OssError> {
        let (host, headers) =
            self.build_request(Method::DELETE, object_name, headers, resources)?;
